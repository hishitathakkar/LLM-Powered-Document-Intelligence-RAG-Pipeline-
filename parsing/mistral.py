...
# Path configuration
PDF_URL = "1cbe8fe7-e08a-46e3-8dcc-b429fc06c1a4.pdf"
...
def replace_images_in_markdown(markdown_str: str, images_dict: dict, pdf_base: str) -> str:
    for img_name, base64_str in images_dict.items():
        markdown_str = markdown_str.replace(
            f"![{img_name}]({img_name})",
            f"![](ocr_output/{pdf_base}/images/{img_name})"
        )
    return markdown_str
...
def process_pdf(pdf_url: str) -> str:
    ...
    return final_markdown  # ✅ Return markdown text for RAG pipeline
